...
def effetsimage():
    if variable == 2:
        ...
        horizontal3 = horizontal2.resize((600,500))
        horizontal3 = ImageTk.PhotoImage(horizontal3)
        # Création du label pour afficher le preset 3
        label_horizontal = Label(fen, image=horizontal3, border=0)
        label_horizontal.place(x=150, y=60)  # Placement du label dans la fenêtre
        label_horizontal.image = horizontal3  # Garde une référence de l'image pour éviter qu'elle ne soit supprimée par python

    if variable ==1:
        vertical1 = Image.open("images.jfif")	# Ouverture de l’image se situant dans le même répertoire que le fichier python
        ...
        vertical3 = vertical2.resize((600,500))
        vertical3 = ImageTk.PhotoImage(vertical3)
        # Création du label pour afficher le preset 3
        label_horizontal = Label(fen, image=vertical3, border=0)
        label_horizontal.place(x=150, y=60)  # Placement du label dans la fenêtre
        label_horizontal.image = vertical3  # Garde une référence de l'image pour éviter qu'elle ne soit supprimée par python

    if variable ==3:
        verthoriz1 = Image.open("images.jfif")	# Ouverture de l’image se situant dans le même répertoire que le fichier python
        ...
        verthoriz4 = verthoriz3.resize((600,500))
        verthoriz4 = ImageTk.PhotoImage(verthoriz4)
        # Création du label pour afficher le preset 3
        label_verthoriz = Label(fen, image=verthoriz4, border=0)
        label_verthoriz.place(x=150, y=60)  # Placement du label dans la fenêtre
        label_verthoriz.image = verthoriz4  # Garde une référence de l'image pour éviter qu'elle ne soit supprimée par python
