...
L, H = image2.size                      # Récupération de la longueur et de la hauteur de l'image d'origine dans les variables L et H
image3 = Image.new("RGB",(int(L * 1.2),H))   # Création d'une nouvelle image vierge 1,2 fois plus haute et de même largeur que l'image d'origine

for y in range(H):                                  # Itération pour chaque colonne
    ...
    for x in range(L):                              # Itération pour chaque pixel de la colonne
        pixel = image2.getpixel((x,y))    #Récupération du pixel de coordonées x,y
        r = pixel[0]        #Récupération de la composante rouge du pixel (RVB) dans la variable r
        v = pixel[1]        #Récupération de la composante verte du pixel (RVB) dans la variable v
        b = pixel[2]        #Récupération de la composante bleue du pixel (RVB) dans la variable b
        image3.putpixel(((x + int(0.1 * L) + N), y),(r,v,b))   #Placement du pixel sur la nouvelle image en tenant compte des marges hautes et basses et du décalage (N)

image3 = image3.crop((int(L * 0.2), 0, int(L * 1.2) - int(L * 0.2), H ))      #"Rognage" des marges hautes et basses de l'image

image3.save("maNouvelleImage.jpg")     #Enregistrement de l'image dans le même répertoire