...
image2 = Image.new("RGB",(int(L * 1.2),H))   # Création d'une nouvelle image vierge 1,2 fois plus haute et de même largeur que l'image d'origine
...
for y in range(H):                                  # Itération pour chaque colonne
    ...
    for x in range(L):                              # Itération pour chaque pixel de la colonne
        ...
        image2.putpixel(((x + int(0.1 * L) + N), y),(r,v,b))   #Placement du pixel sur la nouvelle image en tenant compte des marges hautes et basses et du décalage (N)

image2 = image2.crop((int(L * 0.2), 0, int(L * 1.2) - int(L * 0.2), H ))      #"Rognage" des marges hautes et basses de l'image
